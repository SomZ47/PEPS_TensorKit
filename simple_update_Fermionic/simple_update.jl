...
function get_gates(hams::Vector, τ::Number; TrotterOrder=1)
    ...
    elseif TrotterOrder == 2
        ...
        length(hams) > 1 ? gates[2] = exp(-τ * 0.25 * hams[2]) : nothing
    else
...
function simple_update_1step!(ipeps::iPEPSΓΛ, Dk::Int, gates::Vector{TensorMap}; verbose=1, TrotterOrder=1)
    ...
    if length(gates) >= 2
        for yy in 1:Ly, xx in 1:Lx
            errup1, errup2, errdn1, errdn2, nrmup1, nrmup2, nrmdn1, nrmdn2 = diag_proj_lu2rd!(ipeps, xx, yy, Dk, gates[2])
        ...
        for yy in 1:Ly, xx in 1:Lx
            errup1, errup2, errdn1, errdn2, nrmup1, nrmup2, nrmdn1, nrmdn2 = diag_proj_ru2ld!(ipeps, xx, yy, Dk, gates[2])
            errlis[Nb] = maximum([errup1, errup2, errdn1, errdn2])
